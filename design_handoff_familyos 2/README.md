...
The app is **full-viewport and responsive** around two breakpoints: **`wide` = viewport ≥ 880px** (drives Home and Parent) and **`landscape` = width ≥ 820px AND width > height** (drives the Kid dashboard split). In the prototype both read `window.innerWidth/Height`; production should prefer container/media queries (and may key the Kid split off CSS `orientation: landscape`).
...
- **Wide (≥ 880px) — iPad landscape / desktop web:**
  ...
  - **Kid — portrait / narrow:** a centered single scrolling column (max-width ~640px).
  - **Kid — landscape (≥ 820px AND wider than tall):** a **split view** (max-width ~1180px) — the **left pane** (≈47%, hairline divider on the right) stacks the header, week picker, 7-day calendar, the minutes-gained hero and the redeem button; the **right pane** is the scrolling task list. This is the primary shared-iPad-landscape layout.
